...
async def consultar(payload: AgentRequest):
    try:
        logger.info(f"[abrir_conta_agent] Mensagem recebida: {payload.message}")
        resultado = agente_abertura_conta.invoke(
            {"messages": [HumanMessage(content=payload.message)]}
        )
        mensagem_ia = resultado["messages"][-1]
        resposta = str(mensagem_ia.content)
        logger.info(f"[abrir_conta_agent] Resposta gerada: {resposta}")
        return {"resposta": resposta}
    except Exception as e:
        logger.exception("[abrir_conta_agent] Erro ao processar requisição")
        return JSONResponse(status_code=500, content={"error": str(e)})


@app.get("/health")
async def health():
    return {"status": "ok", "agent": "abrir_conta"}